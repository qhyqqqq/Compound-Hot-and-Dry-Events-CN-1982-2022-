...
<div v-if="isLegendVisible" id="map-legend">
    ...
    >
      <div class="zoomed-image-container">
        <!-- 放大后的图片 -->
        <img
          :src="`/SPI_analyze/${currentImage.filename}`"
          class="zoomed-image"
        />
        <!-- 显示放大图片的流域名称 -->
    ...
    >
      <div class="zoomed-image-container">
        <!-- 放大后的图片 -->
        <img
          :src="`/SPI_mapping/SPI12_${selectedYear}_Map.jpg`"
          class="zoomed-image"
        />
        <!-- 显示放大图片的流域名称 -->
...
export default {
  data() {
    return {
      ...
      imageList: [
        { filename: "SPI12_Annual_Changes_东南诸河.png", region: "东南诸河" },
        { filename: "SPI12_Annual_Changes_海河流域.png", region: "海河流域" },
        { filename: "SPI12_Annual_Changes_长江流域.png", region: "长江流域" },
        { filename: "SPI12_Annual_Changes_黄河流域.png", region: "黄河流域" },
        { filename: "SPI12_Annual_Changes_淮河流域.png", region: "淮河流域" },
        { filename: "SPI12_Annual_Changes_内陆河.png", region: "内陆河" },
        {
          filename: "SPI12_Annual_Changes_松辽河流域.png",
          region: "松辽河流域",
        },
        { filename: "SPI12_Annual_Changes_西南诸河.png", region: "西南诸河" },
        { filename: "SPI12_Annual_Changes_珠江流域.png", region: "珠江流域" },
      ],
      // 新增区域选择相关
      selectedRegions: [],
      regionsOptions: [],
...
,
  ...
  this.map = new Map({
    basemap: "satellite",
    layers: [this.layer,this.geojsonLayer, this.jiuduanLayer ],
  });
  ...
  const imageUrl = `/SPI_mapping/SPI12_${this.selectedYear}_Map.jpg`;  // 获取图片 URL
  const link = document.createElement("a");  // 创建一个 <a> 元素
  link.href = imageUrl;  // 设置图片的 URL
  link.download = `${this.selectedYear}_image.jpg`;  // 设置下载的文件名，并确保后缀为 .jpg
  link.click();  // 触发下载
},
    ...
    adjustTransparency(value) {
      if (this.layer) {
        ...
        // console.log("滑动调整透明度:", value, "图层不透明度:", this.layer.opacity); // 可以取消注释进行调试
      } else {
  ...
  if (selectedYearData) {
    ...
    updateLayer(layerId) {
      ...
      if (this.geojsonLayer) this.map.add(this.geojsonLayer);
      if (this.jiuduanLayer) this.map.add(this.jiuduanLayer);
    },
    // 打开放大图片弹窗
...
/* 或者，如果上面的选择器不生效，尝试更具体的 */
/* .el-slider__runway >>> .el-slider__button-wrapper .el-tooltip {
  display: none !important;
} */
